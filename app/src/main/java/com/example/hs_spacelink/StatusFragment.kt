...
class StatusFragment : Fragment() {
    ...
    companion object {
        ...
        var canceledRoomName: String = ""
        // ★ [정밀 타격용 변수] 취소된 정확한 시간대를 담을 임시 저장소 추가
        var canceledTimeRange: String = ""
    }
                ...
                requireActivity().runOnUiThread {
                    ...
                    if (statusList.isEmpty()) {
                        ...
                        statusList.forEach { (room, time) ->
                            // ★ [이중 정밀 필터 체결 완료]
                            // 취소된 방 이름(canceledRoomName)과 '해당 취소 건의 시간대(canceledTimeRange)'가 동시에 딱 맞아떨어지는 카드만 스킵합니다.
                            // 이름만 같고 시간대가 다른 6층의 나머지 카드들은 이 조건문을 통과해 정상적으로 화면에 그려집니다!
                            if (canceledRoomName.isNotEmpty() && canceledTimeRange.isNotEmpty() &&
                                room.contains(canceledRoomName) && time.trim() == canceledTimeRange.trim()) {
                                Log.d("MOCK_FILTER", "정밀 타깃 매칭 제외 완료: $room ($time)")
                            } else {
    ...
    private fun addStatusCard(container: LinearLayout, roomName: String, timeRange: String) {
        ...
        val badgeLayout = LinearLayout(requireContext()).apply {
            ...
            layoutParams = LinearLayout.LayoutParams(-2, -2).apply { setMargins(0, 12, 0, 0) }
        ...
        val trackButton = com.google.android.material.button.MaterialButton(requireContext()).apply {
            ...
            cornerRadius = 12
            layoutParams = LinearLayout.LayoutParams(-1, -2).apply { setMargins(0, 18, 0, 0) }
            ...
            setOnClickListener {
                ...
                Toast.makeText(requireContext(), "취소 알림 신청이 완료되었습니다.", Toast.LENGTH_SHORT).show()
            }
        }
